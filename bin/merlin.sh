...
if [[ ${CHROMO_NB} == 23 ]] ; then 
    ...
    EXEC1="minx -d ${OUTPUT_DIR_PATH}/c${CHROMO_NB}b/datain.${CHROMO_NB} -p ${OUTPUT_DIR_PATH}/c${CHROMO_NB}b/pedin.${CHROMO_NB} -m ${OUTPUT_DIR_PATH}/c${CHROMO_NB}b/merlin_map.${CHROMO_NB} --error --quiet --pdf --bits:${bit_nb} --minutes:3600 --smallSwap --megabytes:9999 --markerNames --perFamily --rankFamilies --information --tabulate $MODEL_SET"
    echo -e "\nBEWARE: MENDELIAN ERRORS NOT WIPED OUT IN CHROMO 23 (CHROMO X)\n"
    echo -e "\nPROCESS 1/1: $EXEC1\n"
    $EXEC1
    if [[ $MERLIN_ANALYSE_OPTION_CONF == "--npl"  ||  $MERLIN_ANALYSE_OPTION_CONF == "--npl --exp" ]] ; then
    ...
    EXEC1="merlin -d ${OUTPUT_DIR_PATH}/c${CHROMO_NB}b/wiped.dat -p ${OUTPUT_DIR_PATH}/c${CHROMO_NB}b/wiped.ped -m ${OUTPUT_DIR_PATH}/c${CHROMO_NB}b/merlin_map.${CHROMO_NB} --error --quiet --pdf --bits:${bit_nb} --minutes:3600 --smallSwap --megabytes:9999 --markerNames --perFamily --rankFamilies --information --tabulate $MODEL_SET"
