...
tempo.arg.names <- c("r_main_functions", "file", "chromo.nb.set", "lod.cutoff", "path.out", "model")
...
# setwd("C:/Users/gael/Documents/Git_projects/linkage_analysis/work/58/771d66094827292a72cf1cb4ce8c48/caca")
...
if(all(is.na(output.data$Lodscore))){
    ...
    manhattan(output.data, chr = "Chr", snp = "SNP", bp = "Physical.position", p = "Lodscore", logp = FALSE, col = hsv(h = c(4,7)/8, s = 0.4, v = 0.8), ylab = "LODSCORE", genomewideline = 3, suggestiveline = FALSE, main = paste0("WHOLE GENOME\nMODEL: ", model), ylim = range(output.data$Lodscore, na.rm = TRUE))
    for(loop.chromo.nb in 1:length(chromo.nb)){
        tempo.output.data <- output.data[output.data$Chr == chromo.nb[loop.chromo.nb], ]
        manhattan(tempo.output.data, chr = "Chr", snp = "SNP", bp = "Physical.position", p = "Lodscore", logp = FALSE, col = hsv(h = c(4,7)/8, s = 0.4, v = 0.8), ylab = "LODSCORE", genomewideline = 3, suggestiveline = FALSE, main = paste0("CHROMOSOME ", chromo.nb[loop.chromo.nb]), ylim = range(output.data$Lodscore, na.rm = TRUE))
        ...
        g <- ylim(min(output.data$Lodscore, na.rm = TRUE), max(output.data$Lodscore, na.rm = TRUE))
        h <- labs(title = paste0("CHROMOSOME ", chromo.nb[loop.chromo.nb]))
        print(a + b + d + e + f + g + h)
    }
            ...
            for(loop.chromo.nb.set in 1:length(chromo.nb.set)){
                if(any(output.data$Lodscore >= lod.cutoff[i])){
                    if(any(get(paste0("output.data.cut", lod.cutoff[i]))$Chr == chromo.nb.set[loop.chromo.nb.set])){
                        manhattan(subset(get(paste0("output.data.cut", lod.cutoff[i])), Chr == chromo.nb.set[loop.chromo.nb.set]), chr = "Chr", snp = "SNP", bp = "Physical.position", p = "Lodscore", logp = FALSE, col = hsv(h = 5/8, s = 1, v = 1), ylab = "LODSCORE", genomewideline = 3, suggestiveline = FALSE, main = paste0("CHROMOSOME ", chromo.nb.set[loop.chromo.nb.set] , " | LOD SCORE CUT-OFF ", lod.cutoff[i]), ylim = range(output.data$Lodscore, na.rm = TRUE))
                        a <- ggplot(data = tempo.output.data, mapping = aes(x= Physical.position, y = Lodscore))
                        b <- geom_line(size=1)
                        # b <- geom_point(size=1)
                        d <- theme_classic(base_size = 14)
                        ...
                        f <- geom_hline(yintercept = 3, color = "red", alpha = 1, size = 1)
                        g <- ylim(min(output.data$Lodscore, na.rm = TRUE), max(output.data$Lodscore, na.rm = TRUE))
                        h <- labs(title = paste0("CHROMOSOME ", chromo.nb.set[loop.chromo.nb.set]))
                        print(a + b + d + e + f + g + h)
                    }
                }
            }
